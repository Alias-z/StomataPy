...
original_lr = 0.0001
original_n_gpus = 8
lr = original_lr * (n_gpus / original_n_gpus) * (batch_size / original_batch_size) * 100
...
train_pipeline = [
    ...
    dict(type='CutOut', n_holes=4, cutout_ratio=(0.02, 0.03)),
    dict(type='MinIoURandomCrop', min_ious=(0.9, 1.0), min_crop_size=0.1, bbox_clip_border=True),
    dict(
        type='Albu',
    ...
    dict(
        type='RandomChoice',
        transforms=[
            [dict(type='CachedMosaic', img_scale=(crop_size[0] * 4, crop_size[1] * 4), pad_val=0.0, bbox_clip_border=True, prob=1.0),
             dict(type='MinIoURandomCrop', min_ious=(0.9, 1.0), min_crop_size=0.1, bbox_clip_border=True),
             dict(type='Resize', scale=crop_size, keep_ratio=True, interpolation='lanczos'),],
            [dict(type='RandomChoiceResize', scales=[448, 512, 576, 640, 704, 768, 832], resize_type='ResizeShortestEdge', max_size=832),]
        ]),
...
# TTA: https://mmdetection.readthedocs.io/en/latest/user_guides/test.html
